...
data/        — актуальний Excel-файл з даними атестації
...
і застосуються після підтвердження **лише у вашому браузері** (тимчасовий перегляд,
не змінює сайт для інших людей).

### Як оновити дані атестації для всіх (просто, без коду) — `update_site.command`

Найпростіший спосіб для повсякденного оновлення — файл **`update_site.command`**
у корені проєкту:

1. **Замініть Excel-файл**: видаліть старий файл з папки `data/` і покладіть
   туди новий файл атестації `.xlsx` (назва довільна, головне — щоб у `data/`
   залишився **рівно один** `.xlsx`).
2. **Двічі клацніть на `update_site.command`** (у Finder). Відкриється
   термінал, який:
   - перевірить, що в `data/` рівно один `.xlsx`;
   - перевірить і перегенерує `data.js` з цього файлу;
   - якщо все ок — зробить `git commit` і `git push`;
   - якщо є помилка — покаже її і **нічого не відправить**.
3. Якщо все пройшло успішно, побачите повідомлення:
   **"Готово. Netlify оновить сайт за 1–2 хвилини."**

> Якщо при першому запуску macOS заблокує файл ("не вдається перевірити
> розробника") — клацніть правою кнопкою на `update_site.command` →
> **Відкрити** → підтвердіть відкриття.

### Як перевірити сайт після оновлення

1. Зачекайте 1–2 хвилини після повідомлення "Готово".
2. Відкрийте https://atestaciya-yabko.netlify.app/ (можна оновити сторінку
   Cmd+Shift+R, щоб уникнути кешу).
3. Перевірте дату у верхній панелі ("Оновлено: ДД.ММ.РРРР ГГ:ХХ") — вона
   повинна відповідати часу вашого оновлення.
4. Прогрес збірки можна також подивитись на [app.netlify.com](https://app.netlify.com)
   у розділі **Deploys** вашого сайту.

### Ручне оновлення (через термінал)

1. Замініть файл у папці `data/` на новий Excel-файл атестації
   (видаліть старий, покладіть новий — назва файла довільна, головне щоб
   залишився лише один `.xlsx` у цій папці).
2. Закомітьте і запуште зміну:
   ```bash
   git add data/
   git commit -m "Оновити дані атестації"
   git push
   ```
3. Netlify автоматично запустить build (`tools/netlify_build.sh`), який:
   - перегенерує `data.js` з нового Excel-файлу (`tools/build_data.py`);
   - зібере `publish/` з усіма потрібними файлами;
   - опублікує оновлений сайт.
4. Через 1–2 хвилини всі відвідувачі побачать нові дані — перевірити прогрес
   можна на Netlify в розділі **Deploys**.

Локально перевірити результат генерації без публікації:
```bash
python3 tools/build_data.py "data/Атестація Літо 2026.xlsx"
python3 -m http.server 8080
```
